...
namespace BIMFM.Tests
{
    public abstract class BIMFMTestBase : AbpIntegratedTestBase<BIMFMTestModule>
    {
        ...
        protected void UsingDbContext(Action<BIMFMDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<BIMFMDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<BIMFMDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<BIMFMDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<BIMFMDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BIMFMDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<BIMFMDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BIMFMDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<BIMFMDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<BIMFMDbContext, Task<T>> func)
        {
            T result;
