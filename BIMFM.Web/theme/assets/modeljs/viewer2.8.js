...
function onViewerItemSelected(event) {
    ...
    if (dbIdArray.length > 0) {
                ...
                function(result) {
                    ...
                    if (result.properties) {
                        ...
                        for (var i = 0; i < result.properties.length; i++) {
                            ...
                            if (prop.displayName == "Mark" || prop.displayName == "标记") {
                                guid = prop.displayValue;
                            }
                        }
        ...
        $("#baseTb").find("tbody").empty();
        $("#baseTb").append("<tr><td col='2' class='align-center'>暂无数据</td></tr>");
    }
...
function filterByName(name) {
    ...
    oViewer.search(name, function (idArray) {
        if (idArray.length > 0) {
            Isolatemodel(idArray);
            FitModel(idArray);
        }
    });
